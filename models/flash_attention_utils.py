...
def _upad_input(
    ...
    """
    Unpads query, key, and values tensors, \
        using a single dimension for all tokens even though they belong to different batches.

    This function is used instead of `flash_attn.bert_padding.unpad_input` \
        in order to avoid the recomputation of the same intermediary
...
def hf_compatible_fa2_forward(
    ...
    **kwargs,
    ...
    if require_transpose:
        query_length = query_states.shape[2]  # b h l d
